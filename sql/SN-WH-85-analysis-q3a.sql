--- CREATE Table for Cost LESS THN ONE MINUTE
create or replace transient table {db_name}.{sc_name}.MONITOR_AND_ANALYS_WH_STEP_THREE_A AS-- IDRC_SBX_IDROM.CMS_WORK_COMM_PRD.MONITOR_AND_ANALYS_WH_STEP_THREE_A AS
WITH
CTE_QH AS (
    SELECT START_TIME
          ,END_TIME
     ...
     WHERE CLUSTER_NUMBER IS NOT NULL
        ...
        end_time BETWEEN '{start_date}' AND '{end_date}' -- end_time > dateadd(MONTH,-1,CURRENT_DATE())
  ...
  CTE_WMH AS (
    SELECT --*,
     ...
     WHERE  --end_time >=  dateadd(MONTH,-1,CURRENT_DATE())
      end_time BETWEEN '{start_date}' AND '{end_date}' -- >=  dateadd(MONTH,-1,CURRENT_DATE()) -- BETWEEN'{start_date}' AND '{end_date}'--
