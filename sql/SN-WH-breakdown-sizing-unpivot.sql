...
ROUND(count_lt_half_GB / count_queries * 100, 0) AS Percent_less_than_0_5GB ,
ROUND(count_half_to_1GB / count_queries * 100, 0) AS Percent_from_0_5GB_to_1GB ,
--count_half_to_1GB
--count_lt_1GB
/*CASE
...
case warehouse_size
when 'X-Small' then (1 * AVG_QUERY_COST)
when 'Small' then 2 * AVG_QUERY_COST
when 'Medium' then 4 * AVG_QUERY_COST
when 'Large' then 8 * AVG_QUERY_COST
when 'X-Large' then 16 * AVG_QUERY_COST
when '2X-Large' then 32 * AVG_QUERY_COST
when '3X-Large' then 64 * AVG_QUERY_COST
when '4X-Large' then 128 * AVG_QUERY_COST
else 256 * AVG_QUERY_COST
end as AVG_QUERY_COST_WGHT,
*
from wh_sizing as wsh

UNPIVOT(warehouse_sizing FOR sizing IN (Percent_10GB_Plus, Percent_1_to_10GB, Percent_less_than_0_5GB, Percent_from_0_5GB_to_1GB))

--ORDER BY wsh.warehouse_size
ORDER BY
--c.credits_used desc,
case warehouse_size
when 'X-Small' then 1
