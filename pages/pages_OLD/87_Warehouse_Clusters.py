...
if account_selected:
    ...
    df = wb.query('SN-WH-83-clusters-q1.sql',
                  {
                     'start_date': start_date,
                     'end_date'  : end_date,
    ...
    .mark_bar()
    .encode(
        alt.X("TIME_SEC"), #alt.X("TIME_SEC:O"),
        alt.Y("MAX_JOB_ACTIVE_COUNT_MS"),
        alt.Color("CLUSTER_NUMBER"),
        #alt.Size("MAX_JOB_ACTIVE_COUNT_MS"),
        #alt.Tooltip(["WH_AND_SIZE", "AVG_BYTES_LARGE"]), AVG_JOB_ACTIVE_COUNT_MS
        alt.Text('AVG_JOB_ACTIVE_COUNT_MS:Q', format='.1f'),
    )
    .interactive()
    )
