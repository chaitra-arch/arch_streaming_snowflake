...
df_wh = wb.query('SN-WH-83-clusters-q0.sql',
                  {
...
#with col1:
#    st.checkbox("Exclude WHs", key="disabled")
#    wh_selected = st.selectbox('Select to exclude WHs',df_wh['WAREHOUSE_NAME'] , key="disabled") #) + ' MONTH_CREDITS(' + str(df0['CREDITS'][1])+ ')')
#    st.write('You selected:', wh_selected)
#wh_selected = st.selectbox('Select to exclude WHs',df_wh['WAREHOUSE_NAME']) #) + ' MONTH_CREDITS(' + str(df0['CREDITS'][1])+ ')')
...
if account_selected:
    ...
    df_cost = wb.query('SN-WH-85-analysis-q3-C.sql',
                  {
                     'start_date': start_date,
                     'end_date'  : end_date,
                  }
